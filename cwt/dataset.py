...
def load_cwt_dataset(
    root: str,
    img_size: int = 64,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load CWT dataset from pickle files."""
    logger.info("CWT root: %s", root)
    ...
    if current_h != img_size or current_w != img_size:
        logger.info("Resizing CWT data from (%d, %d) to (%d, %d)...", current_h, current_w, img_size, img_size)
    ...
    logger.info("CWT: %d samples loaded. Shape: %s", len(data), data.shape)
